...
const CertificateEditor = ({ onSave, onClose }) => {
  ...
  const [mode, setMode] = useState('idle');            // 'idle' | 'locate' | 'placeSig'
  ...
  const [dragState, setDragState] = useState(null);    // for dragging signature
  ...
  }, [templateImg, namePos, font, fontSize, signatureDataURL, signaturePos, signatureSize]);
  ...
  const handleCanvasMouseDown = (e) => {
    if (mode !== 'idle' || !signaturePos || !canvasRef.current) return;
  ...
  const selectedFont = FONTS.find(f => f.key === font) || FONTS[0];
  const cursorStyle = mode === 'locate' ? 'crosshair' : mode === 'placeSig' ? 'copy' : (dragState ? 'grabbing' : 'default');
      ...
      <div className="flex items-center justify-between px-5 py-3 bg-[#0d1117]/95 border-b border-white/8 shrink-0">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] text-slate-500">Upload template · Place name · Add signature</p>
          </div>
        </div>
            ...
            <div className="relative" style={{ width: templateImg.width * zoom, height: templateImg.height * zoom }}>
              ...
              {mode !== 'idle' && (
                <div className="absolute -top-8 left-1/2 -translate-x-1/2 z-10 bg-amber-500/20 border border-amber-500/40 rounded-lg px-4 py-1.5 text-xs font-bold text-amber-300 whitespace-nowrap">
                  {mode === 'locate' ? '🎯 Click where you want participant names placed' : '✍️ Click where you want the signature placed'}
                </div>
              )}
