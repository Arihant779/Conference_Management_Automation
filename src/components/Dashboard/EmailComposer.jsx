...
const EmailComposer = ({ conf, senderRole = 'organizer', onOpenEmailSettings }) => {
  ...
  const [certSendProgress, setCertSendProgress] = useState('');
          ...
          <div className="lg:col-span-2 space-y-6">
            {step === 'compose' && (
              <div className={cls("rounded-2xl p-6 space-y-6 border transition-all", isDark ? "bg-[#0d1117]/80 border-white/6" : "bg-white border-zinc-200 shadow-sm")}>
                ...
                {genError && (
                  <div className={cls("flex items-center gap-2 rounded-xl px-4 py-3 text-sm", isDark ? "bg-red-500/10 border border-red-500/20 text-red-400" : "bg-red-50 border border-red-200 text-red-600")}>
                    <AlertCircle size={14} /> {genError}
                  </div>
                )}

                {/* ── Certificate attachment ── */}
                <div className={cls("border-t pt-4", isDark ? "border-white/6" : "border-zinc-100")}>
                  <Field label="Attachment" isDark={isDark}>
                    {certConfig ? (
                      <div className={cls("flex items-center gap-3 rounded-xl px-4 py-3 border", isDark ? "bg-emerald-500/10 border-emerald-500/25" : "bg-emerald-50 border-emerald-200")}>
                        <div className={cls("w-8 h-8 rounded-lg flex items-center justify-center shrink-0", isDark ? "bg-emerald-500/20" : "bg-emerald-100")}>
                          <Award size={15} className={isDark ? "text-emerald-400" : "text-emerald-600"} />
                        </div>
                        <div className="flex-1 min-w-0">
                          <p className={cls("text-xs font-semibold", isDark ? "text-emerald-300" : "text-emerald-700")}>Certificate attached ✓</p>
                          <p className={cls("text-[10px]", isDark ? "text-slate-500" : "text-zinc-400")}>Personalized PDF will be generated for each recipient</p>
                        </div>
                        <div className="flex items-center gap-1.5 shrink-0">
                          <button
                            ...
                            className={cls(
                              ...
                              isDark ? "text-slate-500 hover:text-white border-white/10 hover:border-white/20" : "text-zinc-500 hover:text-zinc-900 border-zinc-200 hover:border-zinc-300"
                            )}
                          >
                            Edit
                          </button>
                          <button
                            ...
                            className={cls("p-1 rounded transition-all", isDark ? "text-slate-600 hover:text-red-400 hover:bg-white/5" : "text-zinc-400 hover:text-red-500 hover:bg-zinc-100")}
                          >
                            <X size={12} />
                          </button>
                        </div>
                      ...
                      >
                        <Paperclip size={14} />
                        Attach Certificate
                        <span className={cls("text-[9px] font-normal ml-auto", isDark ? "text-slate-600" : "text-zinc-400")}>Upload template → place name → send</span>
                      </button>
                    )}
                  </Field>
                </div>
            ...
            {step === 'preview' && (
              <div className={cls("rounded-2xl p-6 space-y-6 border transition-all", isDark ? "bg-[#0d1117]/80 border-white/6" : "bg-white border-zinc-200 shadow-sm")}>
                ...
                {/* Send bar */}
                <div className={cls("pt-4 border-t space-y-3", isDark ? "border-white/5" : "border-zinc-100")}>
                  {sendError && (
                    <div className={cls("flex items-center gap-2 rounded-xl px-4 py-3 text-sm", isDark ? "bg-red-500/10 border border-red-500/20 text-red-400" : "bg-red-50 border border-red-200 text-red-600")}>
                  ...
                  {certConfig && (
                    <div className={cls("flex items-center gap-2 rounded-xl px-4 py-2 border", isDark ? "bg-emerald-500/5 border-emerald-500/15" : "bg-emerald-50 border-emerald-100")}>
                      <Award size={13} className={isDark ? "text-emerald-400" : "text-emerald-600"} />
                      <span className={cls("text-[11px] font-semibold", isDark ? "text-emerald-400" : "text-emerald-600")}>Certificate PDF will be attached to each email</span>
                    </div>
                  )}
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    >
                      ...
                      {sending
                        ? (certConfig ? 'Sending certificates…' : 'Sending…')
                        : (certConfig ? 'Send with Certificates' : 'Finalize & Send')}
                    </button>
                  </div>
                </div>
              </div>
          ...
          <div className="space-y-6">
            <div className={cls("rounded-2xl p-6 border transition-all", isDark ? "bg-[#0d1117]/80 border-white/6" : "bg-white border-zinc-200 shadow-sm")}>
              ...
              <div className="space-y-4">
                {[
                  ...
                  { label: 'Certificate', val: certConfig ? '✓ Attached' : 'None' },
                ].map(s => (
                  <div key={s.label}>
            ...
            <div className={cls("rounded-2xl p-6 border transition-all", isDark ? "bg-indigo-500/5 border-indigo-500/15" : "bg-amber-50 border-amber-100")}>
              ...
              <ul className={cls("text-[10px] leading-relaxed space-y-2", isDark ? "text-slate-500" : "text-zinc-600")}>
                ...
                <li>• Attach a <b>certificate template</b> to auto-generate personalized PDFs.</li>
              </ul>
            </div>
          </div>
        </div>
