      ...
      <div className="flex items-center gap-2 mb-3">
        <div className="flex gap-1 p-1 rounded-lg" style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.08)' }}>
          <button onClick={() => setFilterMode('volunteers')} className={cls('px-3 py-1.5 rounded-md text-xs font-semibold transition-all flex items-center gap-1.5')}
            style={filterMode === 'volunteers' ? { background: '#f5c518', color: '#000' } : { color: '#6b7280' }}>
          ...
          <button onClick={() => setFilterMode('all')} className="px-3 py-1.5 rounded-md text-xs font-semibold transition-all"
            style={filterMode === 'all' ? { background: '#fff', color: '#000' } : { color: '#6b7280' }}>
            Conf. Members
          </button>
        </div>
              ...
              <div key={key} onClick={() => !isAdding && handleAdd(c)}
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-1.5 flex-wrap">
                    <span className="text-xs font-semibold text-zinc-200 truncate">{mName(c)}</span>
                  ...
                  <div className="text-[10px] text-zinc-600 truncate">{mEmail(c)}</div>
                  ...
                  {(relevantRoleLabel || domains.length > 0) && isVolTab && (
                    <div className="flex flex-wrap gap-1 mt-1">
                      ...
                      {domains.map(d => <span key={d} className="text-[9px] px-1.5 py-0.5 rounded text-zinc-500" style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)' }}>{d}</span>)}
                    </div>
                  )}
                </div>
