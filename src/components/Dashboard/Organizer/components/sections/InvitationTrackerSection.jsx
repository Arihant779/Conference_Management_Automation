...
import { API_BASE_URL, protectedFetch } from '../../../../../utils/api';
...
const FollowUpButton = ({ inviteId, onSuccess, isDark }) => {
  ...
  const handleFollowUp = async (isScheduled = false) => {
    ...
    setLoading(true);
    try {
      const res = await protectedFetch(`${API_BASE_URL}/api/speakers/follow-up`, {
        method: 'POST',
...
const BulkFollowUpAction = ({ conference, pendingCount, onComplete, isDark }) => {
  ...
  const handleBulkFollowUp = async (isScheduled = false) => {
    ...
    try {
      const res = await protectedFetch(`${API_BASE_URL}/api/speakers/bulk-follow-up`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ 
...
const InvitationTrackerSection = ({ conference, isDark }) => {
  ...
  const fetchInvites = async () => {
    setLoading(true);
    try {
      const res = await protectedFetch(`${API_BASE_URL}/api/speakers/invitations?conference_id=${confId}`);
      const data = await res.json();
      setInvites(data);
    } catch (err) {
      console.error(err);
