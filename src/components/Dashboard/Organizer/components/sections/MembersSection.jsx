            ...
            {filteredMembers.map((m, i) => {
              ...
              return (
                <AnimatedSection key={m.id} delay={0.05 * i} 
                  className={`rounded-2xl px-6 py-4 flex flex-col md:flex-row items-start md:items-center gap-5 transition-all duration-500 border backdrop-blur-xl shadow-md ${
                  ...
                  <div className="w-full md:hidden" /> {/* Spacer for breaking flow clearly */}
                  ...
                  <div className="flex items-center justify-between md:justify-end gap-3 shrink-0 w-full md:w-auto mt-2 md:mt-0 pt-3 md:pt-0 border-t border-white/5 md:border-transparent">
                    ...
                    {isOrganizer ? (
                      ...
                      <div className="flex items-center gap-3">
                        <select value={m.role} onChange={e => updateRole(m.id, e.target.value)}
                          className={cls('text-[10px] md:text-xs font-black px-2 md:px-3 py-1.5 md:py-2 rounded-lg border-2 uppercase tracking-widest cursor-pointer outline-none transition-all', 
