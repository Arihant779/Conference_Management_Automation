...
const Sidebar = ({ nav, section, setSection, isOrganizer, roleLabel, mobileMenuOpen, setMobileMenuOpen }) => {
  ...
  return (
    ...
    <aside className={`fixed md:sticky inset-y-0 top-0 z-50 h-screen transform transition-transform duration-300 ${isOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'} w-64 shrink-0 flex flex-col gap-1 overflow-y-auto no-scrollbar transition-colors duration-500 ${isDark ? 'bg-[#0B0F1A]/95 md:bg-[#0B0F1A]/50' : 'bg-white/95 md:bg-white/80'}`}
      style={{
        backdropFilter: 'blur(20px)',
        borderRight: isDark ? '1px solid rgba(251,191,36,0.1)' : '1px solid rgba(15,23,42,0.15)',
        padding: isOpen ? '24px 16px' : '0 16px', // use isOpen for padding
        paddingTop: '32px',
      }}>
